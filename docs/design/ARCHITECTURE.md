...
- Ship built-in adapters for JSON and plain files.
- Support optional SLHA and xSLHA adapters without requiring their dependencies for core imports.
...
- SLHA and xSLHA structured record parsing/writing
...
`create_default_registry()` returns a fresh registry populated by `register_builtins()`. Module-level helpers such as `get()`, `register()`, and `available_formats()` use a process-local default registry.
...
- `File`: writes plain text content and can read text content when `read_content` is set.
- `SLHA`: reads and writes SLHA using `pyslha`; requires the `slha` extra when used.
- `xSLHA`: reads xSLHA output using `xslha`; requires the `xslha` extra when used. Input writing is not currently supported.
...
- `jportal input.yaml` parses a YAML file and prints the resulting Python dictionary.
- `jportal input.yaml --json` emits JSON.
- `jportal --formats` lists built-in adapter formats.
- `jportal -v` or `jportal --version` prints the installed version.

The CLI should stay separate from Jarvis-HEP workflow execution. It may help inspect or transform Portal-facing data, but it should not take ownership of Jarvis-HEP runtime semantics.
