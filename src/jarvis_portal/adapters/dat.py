...
class DatAdapter:
    ...
    def _write_sync(
        ...
        columns, rows, _source = _read_dat_table(path, spec, context=context)
        ...
        content = _render_dat_table(columns, rows, header=_has_header(spec))
    ...
    def _read_sync(self, context: IOContext, spec: dict[str, Any]) -> dict[str, Any]:
        path = context.path(spec["path"])
        columns, rows, source = _read_dat_table(path, spec, context=context)
        observables: dict[str, Any] = {}
        ...
        if source is None:
            source = _render_dat_table(columns, rows, header=_has_header(spec))
    ...
    *,
    ...
    for line in source.splitlines():
        ...
        if comment and stripped.startswith(comment):
            continue
        parsed_rows.append(stripped.split())
...
def _render_dat_table(columns: list[str], rows: list[list[str]], *, header: bool) -> str:
    width = len(columns) if columns else max((len(row) for row in rows), default=0)
    lines: list[str] = []
    if header:
        lines.append(" ".join(_padded(columns, width)))
    lines.extend(" ".join(_padded(row, width)) for row in rows)
    return "\n".join(lines) + ("\n" if lines else "")
...
def _operation_from_variable(
    ...
    *,
    ...
    if "column" not in variable:
        raise ValueError("DAT input variables require an explicit 'column'.")
...
def _evaluate_expression(context: IOContext, expression: str, data: dict[str, Any]) -> Any:
    if context.evaluate_expression is None:
        raise ValueError(
            "DAT input variable uses 'expression', but IOContext.evaluate_expression is not set."
        )
...
def _ensure_column(
    ...
    *,
    ...
    if isinstance(column, int):
        if column < 0:
            raise ValueError("DAT column index must be non-negative.")
    ...
    if not header and not columns:
        raise ValueError("DAT string columns require header: true or explicit columns.")
    columns.append(text)
    for row in rows:
        _pad_in_place(row, len(columns))
...
def _ensure_row(rows: list[list[str]], row_index: int, width: int) -> None:
    if row_index < 0:
        raise ValueError("DAT row index must be non-negative.")
