...
class CsvAdapter:
    ...
    def _write_sync(
        ...
        columns, rows, _source = _read_csv_table(path, spec, context=context)
        ...
        content = _render_csv_table(columns, rows, header=_has_header(spec))
    ...
    def _read_sync(self, context: IOContext, spec: dict[str, Any]) -> dict[str, Any]:
        path = context.path(spec["path"])
        columns, rows, source = _read_csv_table(path, spec, context=context)
        observables: dict[str, Any] = {}
        ...
        if source is None:
            source = _render_csv_table(columns, rows, header=_has_header(spec))
...
def _read_csv_table(
    ...
    *,
    ...
    parsed_rows = [list(row) for row in csv.reader(StringIO(source))]
    if _has_header(spec):
        if parsed_rows:
...
def _render_csv_table(columns: list[str], rows: list[list[str]], *, header: bool) -> str:
...
def _operation_from_variable(
    ...
    *,
    ...
    if "column" not in variable:
        raise ValueError("CSV input variables require an explicit 'column'.")
...
def _evaluate_expression(context: IOContext, expression: str, data: dict[str, Any]) -> Any:
    if context.evaluate_expression is None:
        raise ValueError(
            "CSV input variable uses 'expression', but IOContext.evaluate_expression is not set."
        )
...
def _ensure_column(
    ...
    *,
    ...
    if isinstance(column, int):
        if column < 0:
            raise ValueError("CSV column index must be non-negative.")
    ...
    if not header and not columns:
        raise ValueError("CSV string columns require header: true or explicit columns.")
    columns.append(text)
    for row in rows:
        _pad_in_place(row, len(columns))
