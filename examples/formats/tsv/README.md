...
TSV is a tabular Jarvis-Portal format with both input and output support.
...
- Identical to CSV except fields are separated by tabs.
- `write_input` reads an existing TSV file or starts from an empty table.
- TSV input writes use Jarvis-HEP-compatible `actions` with `type: Dump`.
